  ...
  private:
    ...
    struct symbol_kind
    {
      enum symbol_kind_type
      {
        ...
        S_WHILE = 27,                            // WHILE
        S_BREAK = 28,                            // BREAK
        S_CONTINUE = 29,                         // CONTINUE
        S_ARRAY = 30,                            // ARRAY
        S_DOT2 = 31,                             // DOT2
        S_YYACCEPT = 32,                         // $accept
        S_s = 33,                                // s
        S_lSentencias = 34,                      // lSentencias
        S_sentencia = 35,                        // sentencia
        S_if = 36,                               // if
        S_markIF = 37,                           // markIF
        S_markIF2 = 38,                          // markIF2
        S_else = 39,                             // else
        S_while = 40,                            // while
        S_markWhile1 = 41,                       // markWhile1
        S_markWhile2 = 42,                       // markWhile2
        S_cond = 43,                             // cond
        S_markOR = 44,                           // markOR
        S_markAND = 45,                          // markAND
        S_operel = 46,                           // operel
        S_expr = 47,                             // expr
        S_declaracion = 48,                      // declaracion
        S_lDims = 49,                            // lDims
        S_lRef = 50,                             // lRef
        S_asignacion = 51                        // asignacion
      };
    };
        ...
        switch (this->kind ())
    {
      ...
      case symbol_kind::S_lRef: // lRef
        value.move< atributos* > (std::move (that.value));
        break;
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, atributos*&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const atributos*& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
switch (yykind)
    {
      ...
      case symbol_kind::S_lDims: // lDims
      case symbol_kind::S_lRef: // lRef
        value.template destroy< atributos* > ();
        break;
    ...
    Parser (void *scanner_yyarg, yy::location& loc_yyarg, class Clase6::Interfaz & intr_yyarg);
    virtual ~Parser ();
    ...
    // Implementation of make_symbol for each token kind.
      ...
      static
      symbol_type
      make_DOT2 (location_type l)
      {
        return symbol_type (token::DOT2, std::move (l));
      }
#else
      static
      symbol_type
      make_DOT2 (const location_type& l)
      {
        return symbol_type (token::DOT2, l);
      }
#endif
    ...
    , location (that.location)
  {
    switch (this->kind ())
    {
      ...
      case symbol_kind::S_lDims: // lDims
      case symbol_kind::S_lRef: // lRef
        value.copy< atributos* > (YY_MOVE (that.value));
        break;
  ...
  Parser::basic_symbol<Base>::move (basic_symbol& s)
  {
    ...
    switch (this->kind ())
    {
      ...
      case symbol_kind::S_lDims: // lDims
      case symbol_kind::S_lRef: // lRef
        value.move< atributos* > (YY_MOVE (s.value));
        break;
