...
%token NEG
// palabras reservadas 
%token IF ELSE WHILE BREAK CONTINUE ARRAY DOT2
/*
forma alterna
%token SUMA '+'
%token ID 'un_id'
...
expr: expr SUMA expr {
    ...
    | lRef ']' {
        ...
        intr.gen->imprimir(temp + " = " + $1->aux + " + " + $1->dir);
        auto temp2 = intr.gen->newTemp(); 
        intr.gen->imprimir( temp2 + "  = " + $1->id + "[" + temp  +"]"); 
        $$ = new atributos(); 
        $$->dir = temp2; 
    }
    ;
...
lDims: lDims ',' expr DOT2 expr {
    // añader min max
    // array arreglo [ 1..y, 1..z, 1..x[x] ...... ]
    intr.gen->tSimbolos->addDim($1->id, $3->dir, $5->dir);
    $$ = $1; 
}

 | ARRAY ID '[' expr DOT2  expr{
        // esta parte ID -> 
        auto ldim = new atributos();
        ldim->id = QString::fromStdString($2);
        intr.gen->tSimbolos->addSimbolo(ldim->id, 
        new Simbolo(ldim->id, @2.begin.line, @2.begin.column ) );
        intr.gen->tSimbolos->addDim(ldim->id, $4->dir, $6->dir  );
        $$ = ldim;
 }
;
...
lRef:   lRef ',' expr {
        // ID [2,3,4,5,6,6]
        auto temp = intr.gen->newTemp(); 
        intr.gen->imprimir(temp + " = " + $1->dir + " - 1 "); 
        auto temp2 = intr.gen->newTemp();
        intr.gen->imprimir(temp2 + " = " + temp + 
                " * " + intr.gen->tSimbolos->getTamDimF($1->id, $1->dim ));
        intr.gen->imprimir($1->aux + " = " + $1->aux + " + "+ temp2);
        $$ = $1; 
        $$->dir = $3->dir; 
        $$->dim++;
    }
    | ID '[' expr {
        ...
        lRef->id = QString::fromStdString($1);
    ...
    ;   
asignacion: ID '=' expr {
    intr.gen->imprimir(QString::fromStdString($1) + QString(" = ") + $3->dir);
}
    | lRef ']' '=' expr{
        auto temp = intr.gen->newTemp(); 
        intr.gen->imprimir(temp + " = " + $1->aux + " + " + $1->dir);
        intr.gen->imprimir($1->id + "[" + temp + "] = " + $4->dir );        
    }
; 
%%
